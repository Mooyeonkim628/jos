...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	if ((r = openfile_lookup(envid, req->req_fileid, &o)) < 0) 
		return r;
	if ((r = file_read(o->o_file, ret->ret_buf, MIN(req->req_n, BLKSIZE), o->o_fd->fd_offset)) < 0) 
		return r;
	o->o_fd->fd_offset += r;
	return r;
}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	if ((r = openfile_lookup(envid, req->req_fileid, &op)) < 0) 
		return r;
	else if ((r = file_write(op->o_file, req->req_buf, MIN(req->req_n, BLKSIZE), op->o_fd->fd_offset)) < 0) 
		return r;
