...
trap_init_percpu(void)
{
	...
	gdt[(GD_TSS0 >> 3) + cpu_idx] = SEG16(STS_T32A, (uint32_t) (this_ts), sizeof(struct Taskstate) - 1, 0);
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if (curenv->env_pgfault_upcall == NULL) {
		cprintf("[%08x] user fault va %08x ip %08x", curenv->env_id, fault_va, tf->tf_eip);
		cprintf("\n");
		print_trapframe(tf);
		env_destroy(curenv);
	}
